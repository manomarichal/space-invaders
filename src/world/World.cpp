...
void World::updateEntities()
{
        for (auto& entity : activeEntities) {
                ...
                if (std::dynamic_pointer_cast<objects::enemies::Enemy>(entity) != nullptr)
                        if (entity->getY() < util::SpaceSettings::yMin + (entity->getYSize() / 2.f))
                                running = false;
        }
...
void World::handleEvents()
{
        ...
        while (index < activeControllers.size()) {
                if (!activeControllers[index]->handleEvents(activeEntities))
                        objectssToDelete.emplace_back(index);
                index++;
        }

        if (endless and endlessStopwatch->isReady())
                createRandomEnemy();
}

void World::onNotify() { running = player->hitpoints > 0; }
...
bool World::isRunning() const { return running; }

bool World::isLevelCompleted() const { return levelCompleted; }

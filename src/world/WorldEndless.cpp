...
{
        if (!endless)
                throw std::runtime_error("world may not create enemies if it is not in endless mode");
        if (spawnLocations.size() == 6)
                spawnLocations.clear(); // all spawn locations are used
        enemiesToDefeat++;
        uint x = 0;
        while (x < 2 or x > 8 or std::find(spawnLocations.begin(), spawnLocations.end(), x) != spawnLocations.end())
                x = rand() % 10;
        ...
        objects::enemies::EnemyFactory::createEnemy(
            util::SpaceSettings::xMin + (util::SpaceSettings::width * (float(x) / 10)),
            util::SpaceSettings::yMax - (util::SpaceSettings::height / 8), eType, *this);
